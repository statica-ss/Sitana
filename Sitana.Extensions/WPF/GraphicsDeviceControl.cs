...
namespace WpfHostedXna
{
   ...
   public class GraphicsDeviceControl : HwndHost
   {
      ...
      private Boolean enableSynchronizedRender = false;

      private DispatcherTimer timer = null;

      private Boolean inRender = false;

      public Boolean EnableSynchronizedRender
      {
         set
         {
            enableSynchronizedRender = value;
            ...
            if ( enableSynchronizedRender )
            {
               timer = new DispatcherTimer(TimeSpan.FromTicks( 500000 ), DispatcherPriority.Normal, new EventHandler(timer_Tick), Dispatcher );
               timer.Start();
            }
         }
      }
      ...
      private void Render()
      {
         ...
         {
            while ( graphicsService.GraphicsDevice.GraphicsDeviceStatus.HasFlag( GraphicsDeviceStatus.Lost ) )
            {
               Thread.Sleep( 10 );
            }
