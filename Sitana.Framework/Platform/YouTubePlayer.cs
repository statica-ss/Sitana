...
namespace Sitana.Framework
{
    ...
    public class YouTubePlayer
    {
        ...
        public YouTubePlayer (CGRect frame)
        {
            ...
            PlayerView.ShouldStartLoad = (webView, request, navType) => 
            {
                if (request.Url.Scheme == @"ytplayer") {
                    ...
                    return HandleHttpNavigationToUrl (request.Url);
                } else if (request.Url.Scheme == @"action") 
                {
                    ProcessAction(request.Url);
                    return false;
                }
                return true;
            };
        ...
        public static string CloseButtonText { private get; set; } = "close";

        #endregion

        #region Private Methods

        void SetupPlayerView (CGRect frame)
        {
        ...
        public void LoadPlayer (Dictionary<string, object> playerParams)
        {
            ...
            embedHTML = embedHTML.Replace ("#!close!#", CloseButtonText);
            PlayerView.LoadHtmlString(embedHTML, NSBundle.MainBundle.BundleUrl);
        }
        ...
        {
            EvaluateJavascript ("var requestFullScreen = iframe.requestFullScreen || iframe.mozRequestFullScreen || iframe.webkitRequestFullScreen;\n  if (requestFullScreen) {\n    requestFullScreen.bind(iframe)();\n  }");
        }
